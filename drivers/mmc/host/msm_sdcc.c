...
typedef struct {
		void (*mmc_bcm_suspend_process)(void);
		void (*mmc_bcm_resume_process)(void);
}mmc_dhd_suspend;

mmc_dhd_suspend   bcm_dhd = {NULL, NULL};		


/*Broadcom add, dhd register stop and start function to mmc*/
void mmc_register_dhd(void *dhd)
{
		bcm_dhd = *((mmc_dhd_suspend *)(dhd));
		printk("shaohua mmc_dhd_register_suspend \n");
}
EXPORT_SYMBOL(mmc_register_dhd);
...
msmsdcc_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	if (mmc) {
		...
		if (!wifi_host && mmc->card && mmc->card->type == MMC_TYPE_SDIO){
			...
			if(bcm_dhd.mmc_bcm_suspend_process) {
					//msmsdcc_wifi_host_off();
					bcm_dhd.mmc_bcm_suspend_process();
					printk("shaohua bcm_dhd.mmc_bcm_suspend_process %p \n", bcm_dhd.mmc_bcm_suspend_process);								
			}
	  }	
      #endif
      #endif /*CONFIG_BCM_WIFI*/
...
if (1) {
		...
		if (wifi_host && mmc->card && mmc->card->type == MMC_TYPE_SDIO){
			...
			if(bcm_dhd.mmc_bcm_resume_process) {
					//msmsdcc_wifi_host_on();
					bcm_dhd.mmc_bcm_resume_process();
					printk("shaohua bcm_dhd.mmc_bcm_resume_process %p \n", bcm_dhd.mmc_bcm_resume_process);
			}
	  }	
#endif
#endif /*CONFIG_BCM_WIFI*/
